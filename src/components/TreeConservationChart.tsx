...
const TreeConservationChart = () => {
  ...
  useEffect(() => {
    generateTreeConservationData(contractpackages).then((result: any) => {
      setTreesData(result);
    });
  ...
  useEffect(() => {
    ...
    pieSeries.slices.template.events.on("click", (ev) => {
      ...
      arcgisMap?.whenLayerView(treeConservationLayer).then((layerView: any) => {
    ...
    <>
      ...
      <CalciteLabel layout="inline">
        <b className="totalTreesNumber">
          {thousands_separators(treesNumber[2])}
          <img
